...
fetch('/my-requests/lecturer', {
  ...
  .then((res) => {
    if (!res.ok) {
      throw new Error("Failed to fetch lecturer requests");
    }
    return res.json();
  })
  .then((items) => {
    ...
    items.forEach((item) => {
      ...
      // Attach detail button event listener
      const detailBtn = row.querySelector(".detail-btn");
      detailBtn.addEventListener("click", () => {
        Swal.fire({
          title: item.asset_name,
          html: `
            <img src="/img/${item.asset_image}" alt="${item.asset_name}" class="img-fluid rounded mb-3" style="max-width: 300px;">
      ...
      console.log("Approving with ID:", approveById);
      // if (!approveById) {
      //   Swal.fire({
      //     icon: "error",
      //     title: "Unauthorized",
      ...
      approveBtn.addEventListener("click", () => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            fetch(`/borrow/${item.request_id}/approve`, { // Use PUT method
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
              },
      ...
      rejectBtn.addEventListener("click", () => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            fetch(`/borrow/${item.request_id}/reject`, { // Use PUT method
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
              },
